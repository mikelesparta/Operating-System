...
int registerB_CPU; // General purpose register V4-EX1
...
int Processor_FetchInstruction() {
	...
	printf("%d %04x\n", registerMAR_CPU, registerPSW_CPU);
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		// Instruction HALT
		case HALT_INST: 
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) // Protected mode V1-EX16
		...
		case IRET_INST: // Return from a interrupt handle manager call
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) // Protected mode V1-EX16
			{
				...
				registerAccumulator_CPU=Processor_CopyFromSystemStack(MAINMEMORYSIZE-3); //V1-EX13
			}				
			else
				Processor_RaiseException(INVALIDPROCESSORMODE);			
			break;		
		...
		//Start V0-EX3
		//Instruction MEMADD
		//The instruction will add operand1 and the contents of the memory cell addressed by
		//memAddress. The result will be stored in the accumulator register.
		case MEMADD_INST: 
			registerMAR_CPU = operand2;

			//Send to the main memory controller
			Buses_write_AddressBus_From_To(CPU, MMU);
		...
		default : 
			...
			Processor_RaiseException(INVALIDINSTRUCTION); //V4-EX3
			registerPC_CPU++;
			break;
	}
...
void Processor_ManageInterrupts() {
	...
	//V2-EX3
	//Interrupts are not checked if interrupts are masked, then we do NOT iterate the interrupts lists
