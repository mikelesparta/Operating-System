	...
	switch (registerCTRL_MMU) {
  	case CTRLREAD:
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)){ // Protected mode
				if (registerMAR_MMU < MAINMEMORYSIZE && registerMAR_MMU >= 0){ //V4-EX1D
					...
					// Fail
					Processor_RaiseException(INVALIDADDRESS); //V4-EX1D
					registerCTRL_MMU |= CTRL_FAIL;
				}
			}
			else // Non-Protected mode
				if (registerMAR_MMU<registerLimit_MMU && registerMAR_MMU >= 0) { //V4-EX1D
